...
public class Avaliação {
	...
	public static void main(String[] args) {
		...
		sistema.alternativas[0][2] = "[3]--" + " [Git pull origin master]";
		...
		sistema.alternativas[2][0] = "[1] --" + " [Criar um branch(ramicação) do repositório atual a partir do arquivo 'principal.java]";
		...
		sistema.alternativas[2][3] = "[4] --" + " [Criar um commit do arquivo 'principal.java' localmente, para posterior envio ao repositório remoto]";
		...
		sistema.alternativas[4][0] = "[1] --" + " [Realizar um commit local e,posteriormente, fazer o envio da versão nova para o servidor principal.]";
		sistema.alternativas[4][1] = "[2] --" + " [Fazer um cópia dos arquivos em uma nova pasta,renomeando esse novo diretório com número de versão acima do atual] ";
		sistema.alternativas[4][2] = "[3] --" + " [Criar uma nova ramificação do repositório local, colocando todos os arquivos da nova versão nas pasta trunk]";
		sistema.alternativas[4][3] = "[4] --" + " [Fazer solicitação de commit para o servidor, de maneira que esta solicitação fique pendente e, assim que existir"
		...
		sistema.alternativas[6][0] = "[1] --" + " [O comanda acima envia alterações para o repositório local, criando uma revisão]";
		sistema.alternativas[6][1] = "[2] --" + " [O comando push não existe na ferramente GIT] ";
		sistema.alternativas[6][2] = "[3] --" + " [O comando acima envia cópia para o repositório local das alterações feitas em um repositório remoto";
		sistema.alternativas[6][3] = "[4] --" + " [O comando acima envia cópia para o repositório remoto das alterações feitas em um repositório local]";
		
		sistema.alternativas[7][0] = "[1] --" + " [git add]";
		sistema.alternativas[7][1] = "[2] --" + " [git status] ";
		sistema.alternativas[7][2] = "[3] --" + " [git branch -a]";
		sistema.alternativas[7][3] = "[4] --" + " [git commit -m 'arquivo git teste adicionado] correto";
		
		sistema.alternativas[8][0] = "[1] --" + " [Adicionar alterações no diretório - usado para especificar em quais terminais remotos os comandos"
		...
		sistema.alternativas[8][1] = "[2] --" + " [Adicionar alterações no diretório - usado para criar terminais remotos - usado para copiar arquivos do repisitório remoto -"
				+ "\r\n usado para exebir alterações no arquivo.] ";
		sistema.alternativas[8][2] = "[3] --" + " [Todos os comandos têm a mesma função dependendo do arquivo no repositório.]";
		sistema.alternativas[8][3] = "[4] --" + " [Todas as alternativas estão incorretas. ]"; 
		
		sistema.alternativas[9][0] = "[1] --" + " [GitHub é uma plataforma de hospedagem de código-fonte e arquivos com controle de versão usando o Git.]";
		...
		for (int i = 0; i < 10; i++) { 
			...
			System.out.print("Digite sua resposta(Escolha um número de 1 a 4): ");
